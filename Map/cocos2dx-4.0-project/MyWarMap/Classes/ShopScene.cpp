...
void ShopScene::displayCoin() {
    auto visibleSize = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin();
    auto coinLabel = Label::createWithTTF("", "fonts/Marker Felt.ttf", 30);
    coinLabel->setColor(Color3B(255, 0, 0));
    coinLabel->setString(StringUtils::format("Coins: %d", coin));
    coinLabel->setPosition(Vec2(96,416));
    this->addChild(coinLabel, 4);
}
//------------------------------------------------------------------//
//购买函数
void ShopScene::purchasePlayer() {
